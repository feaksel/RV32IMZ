...
read_libs "$LIB_PATH/${LIB_VARIANT}/lib/sky130_osu_sc_18T_ms_TT_1P8_25C.ccs.lib"
...
#read_physical -lef "$LIB_PATH/${LIB_VARIANT}/lef/sky130_osu_sc_18T_ms.lef"
...
if {[file exists "${MACRO_DIR}/core_macro/core_macro_netlist.v"]} {
    read_hdl -v2001 "${MACRO_DIR}/core_macro/core_macro_stub.v"
...
if {[file exists "${MACRO_DIR}/mdu_macro/mdu_macro_netlist.v"]} {
    read_hdl -v2001 "${MACRO_DIR}/mdu_macro/mdu_macro_stub.v"
...
# 1. Prevent the DESIGN from being optimized
if {[llength [get_db designs core_macro]] > 0} {
    set_db [get_db designs core_macro] .preserve true
    set_db [get_db designs core_macro] .ungroup_ok false   ;# <--- ADD THIS
    set_dont_touch [get_db designs core_macro]
}

# 2. Prevent the INSTANCE from being ungrouped (CRITICAL)
# This forces the specific instance 'u_core_macro' to stay intact
set_db [get_db insts u_core_macro] .preserve true
set_db [get_db insts u_core_macro] .ungroup false       ;# <--- ADD THIS

if {[llength [get_db designs mdu_macro]] > 0} {
    set_db [get_db designs mdu_macro] .preserve true
    set_db [get_db designs mdu_macro] .ungroup_ok false   ;# <--- ADD THIS
    set_dont_touch [get_db designs mdu_macro]
}

# Protect the mdu instance as well
set_db [get_db insts u_mdu_macro] .preserve true
set_db [get_db insts u_mdu_macro] .ungroup false       ;# <--- ADD THIS
#===============================================================================
# Read Constraints
#===============================================================================
